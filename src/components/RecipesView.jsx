...
// ── Single image upload + OCR section ────────────────────────────────────────
function ImageSection({ label, image, setImage, text, setText, scanning, setScanning, progress, setProgress, active, onActivate }) {
  ...
  const handleFile = useCallback(async (file) => {
    ...
    setText('');
    try {
      const result = await ocrFile(file, setProgress);
      setText(result);
    } catch { setText(''); }
    setScanning(false);
  }, [setImage, setScanning, setProgress, setText]);

  // expose handleFile so parent can call it for paste
  useEffect(() => { onActivate(handleFile); }, [handleFile, onActivate]);

  return (
    <div onClick={() => onActivate(handleFile)}>
      ...
      <div
        ...
        style={{
          border: `2px dashed ${active ? '#C4614A' : '#C4B5A0'}`,
          backgroundColor: active ? '#FFF3EE' : '#FAF3EA',
          minHeight: image ? undefined : '100px',
      ...
      >
        {image ? (
          <img src={image} alt={label} className="w-full rounded-xl object-contain" style={{ maxHeight: '160px' }} />
        ) : (
          <>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={active ? '#C4614A' : '#C4B5A0'} strokeWidth="1.5">
              <rect x="3" y="3" width="18" height="18" rx="2"/>
              <circle cx="8.5" cy="8.5" r="1.5"/>
              <polyline points="21 15 16 10 5 21"/>
            </svg>
            <p className="mt-2 text-xs font-medium" style={{ color: active ? '#C4614A' : '#A67C60' }}>
              Tap to upload, drag & drop, or paste screenshot
            </p>
          </>
        )}
      </div>
      ...
      {scanning && (
        <div className="mt-2 rounded-xl px-3 py-2 flex items-center gap-2" style={{ backgroundColor: '#F0E6D3' }}>
...
function AddRecipeModal({ onSave, onClose }) {
  ...
  // track which section is "active" for paste
  const [activeSection, setActiveSection] = useState('ingredients');
  const ingHandlerRef = useRef(null);
  const insHandlerRef = useRef(null);

  // paste support — routes to whichever section is active
  useEffect(() => {
    const handlePaste = (e) => {
      ...
      for (const item of items) {
        if (item.type.startsWith('image/')) {
          ...
          if (!file) break;
          if (activeSection === 'ingredients' && ingHandlerRef.current) ingHandlerRef.current(file);
          else if (insHandlerRef.current) insHandlerRef.current(file);
          break;
        }
      }
    };
    ...
    >
      ...
      >
        ...
        <div className="mx-6 mt-3 rounded-xl px-3 py-2 text-xs text-center" style={{ backgroundColor: '#F0E6D3', color: '#7A4030' }}>
          📋 Use <strong>Cmd+Ctrl+Shift+4</strong> to copy a screenshot, then paste it into either section below
        </div>

        <div className="px-6 py-4 flex flex-col gap-5">
          {/* ingredients */}
          <div onClick={() => setActiveSection('ingredients')}>
            <ImageSection
              label="📋 Ingredients"
              ...
              onActivate={fn => { ingHandlerRef.current = fn; }}
            />
          </div>

          {/* instructions */}
          <div onClick={() => setActiveSection('instructions')}>
            <ImageSection
              label="📝 Instructions"
              ...
              onActivate={fn => { insHandlerRef.current = fn; }}
            />
          </div>
    ...
    <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: '#FFF0E6', border: '1.5px solid #E8C5A8' }}>
      <div className="flex items-center">
        ...
        <button
          ...
          className="flex flex-col items-center px-3 py-3 transition-opacity flex-shrink-0"
        ...
        <div className="flex flex-col gap-3">
          {filtered.map(recipe => (
            <RecipeCard
              ...
              onDelete={onDelete}
              onUpvote={onUpvote}
              myUpvoted={myRecipeUpvotes.has(recipe.id)}
            />
          ))}
        </div>
      )}
